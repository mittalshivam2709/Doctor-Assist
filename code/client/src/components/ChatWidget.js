...
const ChatWidget = () => {
  ...
  const [newMessages, setNewMessages] = useState({ SingleChat: false, HelpChat: false });
  ...
  >
    ...
    <div className={`chat-widget ${expanded ? 'expanded' : ''}`} style={widgetStyle}>
        ...
        {expanded ? 
        <div className='close'>
          <button onClick={() => {setActiveTab('SingleChat'); setNewMessages(prev => ({ ...prev, SingleChat: false }));}} style={{backgroundColor: activeTab === 'SingleChat' ? 'rgba(120, 120, 255, 1)' : 'rgba(95, 95, 255, 1)',padding:'10px 25px 10px 25px', borderRadius:'15px 15px 0 0', marginRight:'10px'}}>EMT Assist {newMessages.SingleChat && <span className="notif-dot"></span>}</button>
          <button onClick={() => {setActiveTab('HelpChat'); setNewMessages(prev => ({ ...prev, HelpChat: false }));}} style={{backgroundColor:activeTab === 'HelpChat' ? 'rgba(120, 120, 255, 1)' : 'rgba(95, 95, 255, 1)',padding:'10px 25px 10px 25px', borderRadius:'15px 15px 0 0', marginRight:'25%'}}>Help Centre {newMessages.HelpChat && <span className="notif-dot"></span>} </button>
          <button onClick={toggleExpansion} className="chat-widget-button"><MySign/></button>
        </div>
        :  
