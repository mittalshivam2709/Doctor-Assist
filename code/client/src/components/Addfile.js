...
const Addfile = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5002/upload_documents', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  return (
    <div>
      ...
      <div>
        <div>
          <h1 style={{ color: 'rgba(85,85,251,1)', fontWeight: 'bold', fontSize: '30px', position: 'absolute', left: '59%', transform: 'translateX(-50%)' }}>
            File Upload
          </h1>
          <br />
          <br />
          <div style={{ position: 'absolute', top: '30%', left: '45%' }}>
            ...
            <br />
            <button onClick={handleUpload} style={{ padding: '20px', borderRadius: '10px', backgroundColor: 'rgba(2,73,255,1)', color: 'rgba(255,255,255,1)', height: '67px', top: '642px', width: '205px', border: 'none', cursor: 'pointer', marginLeft: '170px' }}>
              SAVE
            </button>
          </div>
          <br />
